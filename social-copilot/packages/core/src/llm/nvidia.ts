...
function extractOpenAICompatibleContent(data: unknown): string {
  if (!isPlainObject(data)) {
    throw new Error(`Invalid response structure: expected object, got ${formatType(data)}`);
  }

  const choices = (data as { choices?: unknown }).choices;
  if (!Array.isArray(choices) || choices.length === 0) {
    throw new Error('Invalid response structure: missing non-empty choices array');
  }

  const firstChoice = choices[0];
  if (!isPlainObject(firstChoice)) {
    throw new Error(`Invalid response structure: choices[0] expected object, got ${formatType(firstChoice)}`);
  }

  const message = (firstChoice as { message?: unknown }).message;
  if (!isPlainObject(message)) {
    throw new Error(`Invalid response structure: choices[0].message expected object, got ${formatType(message)}`);
  }

  const content = (message as { content?: unknown }).content;
  if (typeof content !== 'string') {
    throw new Error(`Invalid response structure: choices[0].message.content expected string, got ${formatType(content)}`);
  }
...
export class NvidiaProvider implements LLMProvider {
  ...
  constructor(config: NvidiaProviderConfig) {
    ...
    this.baseUrl = normalizeBaseUrl(config.baseUrl || 'https://integrate.api.nvidia.com', {
  ...
  async generateReply(input: LLMInput): Promise<LLMOutput> {
    ...
    if (!response.ok) {
      ...
      const errorMessage = redactSecrets((errorData as { error?: { message?: string } }).error?.message || `status ${response.status}`);
  ...
  private parseNonReplyResponse(
    content: string,
    styles: ReplyStyle[],
    task: Exclude<LLMInput['task'], 'reply'>
  ): ReplyCandidate[] {
    try {
